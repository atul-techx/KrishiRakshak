...
const d=data?.result?.disease?.suggestions?.[0],c=data?.result?.crop?.suggestions?.[0];
...
return json({provider:'Google Gemini',assessment:visualAssessment,secondOpinion:true,kindwiseAssessment:{crop:c?.name||'Unknown',finding:d?.name||'Uncertain'},candidates:(data.result.disease?.suggestions||[]).slice(0,3).map(x=>({name:x.name,probability:x.probability})),sources:[d?.details?.wiki_url,c?.details?.wiki_url].filter(u=>typeof u==='string'&&/^https:\/\//.test(u))});
}
